...
const authSupabaseService = {
  ...
  // Login via CNPJ (mapeia CNPJ para email)
  autenticarCnpj: async (cnpj, senha) => {
    try {
      ...
      // Formata o CNPJ para email (ex.: 05336475000177@fitinbox.com)
      const cnpjLimpo = cnpj.replace(/\D/g, "");
      const email = `${cnpjLimpo}@fitinbox.com`;

      // Chama o login padrão
      const result = await authSupabaseService.login(email, senha);
      if (!result.success) return result;

      console.log('✅ Login via CNPJ realizado com sucesso:', {
        user: result.user.email,
        empresa: result.empresa.nome_empresa
      });
      ...
      return {
        success: false,
        error: error.message || "Erro desconhecido ao fazer login com CNPJ."
      };
    }
  },
