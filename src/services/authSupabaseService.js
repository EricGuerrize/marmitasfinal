...
export const authSupabaseService = {
    ...
    // Banco de dados mock em localStorage
    getEmpresasDatabase: () => {
      try {
    ...
    buscarEmpresaPorCnpj: (cnpj) => {
      const empresas = this.getEmpresasDatabase();
      const cnpjLimpo = cnpj.replace(/\D/g, '');
    ...
    autenticar: async (cnpj, senha) => {
      try {
        ...
        const empresa = this.buscarEmpresaPorCnpj(cnpjLimpo);
        ...
        if (empresa.senha !== senha) {
          ...
          if (index >= 0) {
            ...
            this.saveEmpresasDatabase(empresas);
          }
        ...
        sessionStorage.setItem('cnpj', this.formatarCnpj(cnpjLimpo));
    ...
    registrarEmpresa: async (cnpj, senha, dadosEmpresa) => {
      try {
        ...
        const empresaExistente = this.buscarEmpresaPorCnpj(cnpjLimpo);
        if (empresaExistente) {
          return {
            success: false,
        ...
        const dadosReceita = await this.buscarDadosReceita(cnpjLimpo);
        ...
        const salvou = this.saveEmpresasDatabase(empresas);
    ...
    // Simula busca na Receita Federal
    buscarDadosReceita: async (cnpj) => {
